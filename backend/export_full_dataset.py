...
export_path_root = "e:/scada/full_microgrid_dataset.csv"
export_path_sample = "e:/scada/sample_datasets/full_microgrid_dataset.csv"
...
def export_data():
    ...
    print("Fetching all microgrid telemetry records...")
    cursor.execute("SELECT * FROM scada_historian ORDER BY timestamp ASC")
    rows = cursor.fetchall()
    conn.close()

    if not rows:
        print("No telemetry records found in the database.")
        return
